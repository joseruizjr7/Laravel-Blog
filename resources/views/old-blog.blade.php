...
@section('content')
	...
	<section class="posts container">
            ...
            <article class="post no-image">
                <div class="content-post">
                    <header class="container-flex space-between">
                        ...
                        <div class="post-category">
                            <span class="category text-capitalize">
								<a href="{{ route('category.show', $post->category) }}" style="text-decoration:none; color:#fff;">{{ $post->category->name }}</a>
							</span>
                        </div>
                    </header>
                    <h1>{{ $post->title }}</h1>
                    ...
                    <footer class="container-flex space-between">
                        <div class="read-more">
                            <a href="{{ route('blog.post', ['post' => $post]) }}" class="text-uppercase c-green">Leer más</a>
                        </div>
                        <div class="tags container-flex">
							@foreach ($post->tags as $tag)
								<span class="tag c-gray-1 text-capitalize"><a href="{{ route('tags.show', $tag->name) }}">#{{ $tag->name }}</a></span>
							@endforeach
                        </div>
                    </footer>
                </div>
		...
		<article class="post w-image">
			<figure><img src="{{ asset('assets/img/img-post-1.png') }}" alt="" class="img-responsive"></figure>
			<div class="content-post">
				<header class="container-flex space-between">
					<div class="date">
